...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await findByCpf(cpf);
    ...
    console.error("[GET /colaboradores/[cpf]]", error);
...
export async function PUT(
  ...
  try {
    ...
    const { data: existing, error: fetchError } = await findByCpf(cpf);
    ...
    // CPF não pode ser alterado via PUT
    ...
    // 5. Auditoria
    await logEditar(user.re, cpf, existing.nome ?? cpf, changedFields);

    return NextResponse.json({ data: mapRow(updated) });
  } catch (error) {
    console.error("[PUT /colaboradores/[cpf]]", error);
...
export async function DELETE(
  ...
  try {
    ...
    // 3. Auditoria
    await logRemover(user.re, cpf, existing.nome ?? cpf);

    return NextResponse.json({ message: "Colaborador removido com sucesso" });
  } catch (error) {
    console.error("[DELETE /colaboradores/[cpf]]", error);
