...
export default function DashboardPage() {
  ...
  // Grupos de etapas — estados SEPARADOS para cada seção (evita collapse compartilhado)
  // Ambos iniciam com todos os grupos colapsados (preenchidos pelo useEffect abaixo)
  const [collapsedCronogramaGrupos, setCollapsedCronogramaGrupos] = useState<Set<number>>(new Set());
  const [collapsedDashboardGrupos,  setCollapsedDashboardGrupos]  = useState<Set<number>>(new Set());

  const toggleCronogramaGrupo = (id: number) =>
    setCollapsedCronogramaGrupos((prev) => {
  ...
  // Inicializa ambas as seções com todos os grupos colapsados (executa uma única vez)
  const gruposInitialized = useRef(false);
  useEffect(() => {
    if (!gruposInitialized.current && etapasPorGrupo?.grupos.length) {
      const allIds = new Set(etapasPorGrupo.grupos.map((g) => g.id));
      setCollapsedCronogramaGrupos(allIds);
      setCollapsedDashboardGrupos(allIds);
      gruposInitialized.current = true;
    }
  }, [etapasPorGrupo]);
                ...
                <Card className="glass-card">
                  ...
                  <CardContent className="p-0">
                    <GanttCronograma
                      ...
                      collapsedGrupos={collapsedDashboardGrupos}
                      onToggleGrupo={toggleDashboardGrupo}
                    />
                  </CardContent>
                </Card>
              </div>
                        ...
                        return (
                          <div className="space-y-6">
                            {etapasPorGrupo.grupos.map((grupo) => {
                              ...
                              const isCollapsed = collapsedCronogramaGrupos.has(grupo.id);
                              return (
                                <div key={grupo.id} className="rounded-xl border border-primary/20 bg-primary/5 p-4 space-y-4">
                                  <button
                                    type="button"
                                    onClick={() => toggleCronogramaGrupo(grupo.id)}
