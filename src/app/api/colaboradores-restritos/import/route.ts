...
import { buildHeaderMap, sanitizeCPF, type RawRow } from "@/lib/import-utils";
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const tipoDemissao = toStr(getBySchema(row, headerMap, "tipo_demissao") ?? "");
      const motivoDemissao = toStr(getBySchema(row, headerMap, "motivo_demissao") ?? "");
    ...
    const { data: existing } =
      cpfsParaBuscar.length > 0
        ? await db.from("colaboradores_restritos").select("cpf").in("cpf", cpfsParaBuscar)
        : { data: [] };
    ...
    await registrarLog(
      user.re,
      "IMPORTAR",
      `Importação de ${records.length} colaboradores restritos (${inseridos} novos, ${atualizados} atualizados)`,
    );
    ...
    console.error("[POST /colaboradores-restritos/import]", msg);
